...
public class LeadPoolService {
	...
	public List<LeadPoolDTO> page(BasePageRequest request) {
		...
		List<String> poolIds = pools.stream().map(LeadPoolDTO::getId).toList();
		...
		Map<String, LeadPoolPickRule> pickRuleMap = pickRules.stream().collect(Collectors.toMap(LeadPoolPickRule::getPoolId, pickRule -> pickRule));
		...
		Map<String, LeadPoolRecycleRule> recycleRuleMap = recycleRules.stream().collect(Collectors.toMap(LeadPoolRecycleRule::getPoolId, recycleRule -> recycleRule));
		pools.forEach(pool -> {
			...
			LeadPoolRuleDTO rule = LeadPoolRuleDTO.builder().limitOnNumber(pickRule.getLimitOnNumber()).pickNumber(pickRule.getPickNumber())
					.limitPreOwner(pickRule.getLimitPreOwner()).pickIntervalDays(pickRule.getPickIntervalDays())
					.expireNotice(recycleRule.getExpireNotice()).noticeDays(recycleRule.getNoticeDays())
					.operator(recycleRule.getOperator()).conditions(JSON.parseArray(recycleRule.getCondition(), RuleConditionDTO.class)).build();
			pool.setRule(rule);
		});
		return pools;
	}
	...
	public void checkPick(String id) {
		...
		wrapper.eq(LeadPoolRelation::getPoolId, id).eq(LeadPoolRelation::getPicked, false);
