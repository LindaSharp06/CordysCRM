...
export default function useTreeSelection(selectedModuleProps: SelectedModuleProps) {
  ...
  const moduleTree = ref<TreeSelectOption[]>(modulesTree);
  ...
  // 设置节点
  function setNode(nodeData: TreeSelectOption, checked: boolean) {
    selectedModulesMaps.value[nodeData.id as string] = {
      selectAll: checked,
      selectIds: new Set(),
      excludeIds: new Set(),
      count: nodeData?.count as number,
    };
  }

  // 选中当前节点 && 取消当前节点
  function selectParent(nodeData: TreeSelectOption, isSelected: boolean) {
    setNode(nodeData, !isSelected);
  }

  // 初始化左侧模块节点选中当前以及子节点
  function processAllCurrentNode(node: TreeSelectOption, check: boolean) {
    if (node?.children && node?.children?.length) {
      node.children.forEach((childrenNode: TreeSelectOption) => processAllCurrentNode(childrenNode, check));
    }
    setNode(node, check);
  }

  // 选中当前节点以及子节点
  function checkNode(_checkedKeys: Array<string | number>, checkedData: CheckedNodes) {
    const { checked, node } = checkedData;
    processAllCurrentNode(node as TreeSelectOption, checked as boolean);
  }

  // 全选全部&取消全选
  function setSelectAll(tree: TreeSelectOption[], checkedAll: boolean) {
    tree.forEach((node) => {
  ...
  // 设置模块树
  function setModuleTree(tree: TreeSelectOption[]) {
